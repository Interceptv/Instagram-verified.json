/*! P6.0 | (c) Open Foundation and other contributors */
...
function Connection(messageHandler) {
  ...
  ServerMessage.prototype.popFloat = function() {
...
function onLogin() {
  Game.queueLogin(secret, 'atlantus', secret, esalaid, myip, erank);
}
...
function onGlobal(data) {
  Game.RequestGlobal(data);
}

function onChatSubmit() {
  var chat_text = document.querySelector('#lonechat').innerText;
   if(document.querySelector('#lonechat2').innerText != "null")
  {
  var chat_text = ""+ document.querySelector('#lonechat2').innerText +" " + document.querySelector('#lonechat').innerText + "";
  }
  $('#lonechat').empty();
  Game.RequestGlobal(chat_text);
}
...
Game.run = function(canvas) {
  ...
  updateStatus("Carregando files...");
  try
  {
    $('#ldx').text("ConteÃºdo em ciclo!");
    document.getElementById("loadpercent").style.width = "14%";
  }
  catch{}
    this.canvas = canvas;
...
$('#ldx').text("Conectando ao servidor...");
try
  {
document.getElementById("loadpercent").style.width = "20%";
  }
  catch{}
}.bind(Game);
...
Game.queueLogin = function(username, look, password, salaid, meuip, rank) {
  ...
  if (this.communication != null) {
    this.doLogin(username, look, password, salaid, meuip, rank);
  } 
  else {
...
Game.RequestGlobal = function(chat) {
  if (this.currentRoom != null && this.currentRoom.ready) {
    this.communication.RequestGlobal(chat);
  }
};
...
Game.SalaGlobal = function() {
  if (this.currentRoom != null && this.currentRoom.ready) {
    this.communication.SalaGlobal();
  }
};

Game.doLogin = function(username, look, password, salaid, meuip, rank) {
  if (this.communication != null) {
    this.communication.doLogin(username, look, password, salaid, meuip, rank);
  }
};
...
Game.setMap = function(cols, rows, doorX, doorY, heightmap) {
  ...
  this.currentRoom.prepare().then(function () {
    updateStatus("-- carregando 100%");
    try
    {
$('#ldx').text("Carregando Recursos");
    
var qtd = 999999;
var el = $('#loadpercent'),
w = el.width();
el.animate({ width: '50%' }, 10);
function pisca(){
  //onGlobal('auth######');
  startauth();
el.animate({ width: w * 1.5 }, 10);
$(document).ready(function() {
el.animate({ width: '100%' }, 0);
      qtd = 0;
 $('#loneloading').fadeOut(900);
})
    if (qtd--) {setTimeout( pisca, 400 )} else{};
}
    }
    catch{}
...
Game.handleOpenConnection = function() {
  ...
  try
  {
$('#ldx').text("Conectado a Rede!");
  }
catch{}
setTimeout(function(){ 
  try
  {
document.getElementById("loadpercent").style.background = "yellow";
$('#ldx').text("Pico de conexÃµes, iremos mudar sua rota :)");
document.getElementById("loadpercent").style.width = "50%";
  }
  catch{}
 }, 4000);
 try{
document.getElementById("loadpercent").style.width = "18%";
 }
  catch{}
  this.communication = new Communication(this);
  if (this.queuedLogin) {
    this.doLogin(this.username, this.look, this.password);
  onLogin();
  } 
this.queuedLogin = false;
  onLogin();
...
Communication.OUTGOING_SALA_GLOBAL = 14;
...
Communication.prototype.doLogin = function(username, look, password, salaid, meuip, rank) {
  ...
  message.appendString(salaid);
  message.appendString(meuip);
  message.appendInt(rank);
...
Communication.prototype.RequestGlobal = function(chat) {
  if (chat.length > 0) {
...
Communication.prototype.SalaGlobal = function() {
  var message = new ClientMessage(Communication.OUTGOING_SALA_GLOBAL);
  this.game.connection.sendMessage(message);
};
...
Communication.prototype.handleMessage = function(data) {
  ...
  switch (request.id)
  {
    ...
    case Communication.INCOMING_PLAYER_WAVE:
      ...
      break;   
       case Communication.INCOMING_SALA_GLOBAL:
      this.handleSalaGlobal(request);
      break;
  }
};
...
Communication.prototype.handlePlayers = function(request) {
  ...
  for (var i = 0; i < count; i++) {
    ...
    var login = request.popString();
    var avatar = request.popString();
    var salaid = request.popString();
    var meuip = request.popString();
    var rank = request.popInt();
    var newnick = request.popString();
    var ghost = request.popString();
    if (this.game.currentRoom != null) {
      this.game.currentRoom.setPlayer(id, x, y, z, rot, name, look, userid, avatar, login, salaid, meuip, rank, newnick, ghost);
    }
  }
};
...
Communication.prototype.handleSalaGlobal = function(request) {
  var userId = request.popInt();
  if (this.game.currentRoom != null) {
    this.game.currentRoom.SalaGlobal(userId);
  }
};
function Player(id, x, y, z, rot, name, look, userid, avatar, login, newnick, ghost)
{
  this.id = id;
  this.ready = false;
  this.waveCounter = 0;
  ...
  this.updateParams(x, y, z, rot, name, look, userid, avatar, login, newnick, ghost);
  this.statusses = {};
}
...
Player.prototype.updateParams = function(x, y, z, rot, name, look, userid, avatar, login, newnick, ghost) {
  this.x = x;
  this.y = y;
  this.targetX = x;
  this.targetY = y;
...
Room.prototype.setPlayer = function(id, x, y, z, rot, name, look, userid, avatar, login, salaid, meuip, rank, newnick, ghost) {
  ////ADICIONAR ONLINE
  var player = this.getPlayer(id);
  if (player == null) {
  var p = new Player(id, x, y, z, rot, name, look, userid, avatar, login, salaid, meuip, rank, newnick, ghost);
    this.players[id] = p;
    meid = userid;
    if(userid == myid) {
    if (nickgen == null){nickgen = newnick; }
    heynome = name;
    onGlobal();
    onSave();
     viewp = id; schange = "block"; schange2 = "inline-block";
  }
  else
  {
    nickgen = "...";
  }
    if(userid != myid) {heynome = null; schange = "none";schange2 = "none";}

    $('#loneusers').append("<script>\
$(\'#loneuser"+ id +"\').click(function(){\
 document.getElementById(\"materializebg\").style.display = \'block\';\
document.getElementById(\"lojarender\").style.display = 'none';\
      $('#materializebg').append(\"<div id=\'perfilload\'><center class=\'centermod\'>\
      <div id=\'bb"+ userid +"\' class=\'bb\'><div onclick=\'pvopen(this)\' class=\'closemodal\'>X</div>\
      <div class=\'userinfo\'>"+ meid +"  "+ login +"</div>\
       <div class=\'posmk\' style=\'color:white;\'>\
      <div id=\'resultado"+ userid +"\' style=\'display:none;background: #118b48;\' class=\'userinfo\'></div>\
      <div id=\'ftuser\' class=\'ftuser\' style=\'background:url("+ avatar +"); background-size:100% 100%;\'></div>\
      <div class=\'rndft\' style=\'display:"+ schange2 +"\'></div>\
      <div class=\'rndft\' style=\'display:"+ schange2 +"\'></div>\
      <div class=\'rndft\' style=\'display:"+ schange2 +"\'></div>\
      <div class=\'rndft\' style=\'display:"+ schange2 +"\'></div>\
      <div class=\'rndft\' style=\'display:"+ schange2 +"\'></div>\
      <div class=\'rndft\' style=\'display:"+ schange2 +"\'></div><br>\
      <div id='nocso"+ id +"' style='display:none;'></div>\
      <div style=\'display:"+ schange +"\'>Nome</div><input type=\'text\' autocomplete=\'off\' id=\'meunome"+ userid +"\' class=\'nminp\' style=\'display:"+ schange +"\' value=\'...\'>\
      <div style=\'display:"+ schange +"\'>Avatar</div><input type=\'text\' id=\'meuavatar"+ userid +"\' class=\'nminp\' style=\'display:"+ schange +"\' value=\'...\'>\
      <div id=\'asave\' onclick=\'menubotao(this)\' class=\'nmsv\' style=\'display:"+ schange +"\'>Salvar</div></div>\
      <div class=\'perfilsize\'> \
      <div id=\'add"+ userid +"\' onclick=\'menubotao(this)\' class=\'botaoperfil\'>Adicionar +</div><div id=\'esp@"+ userid +"\' onclick=\'menubotao(this)\' class=\'botaoperfil\'>Especiais</div>\
      <div id=\'trans@"+ userid +"\' onclick=\'menubotao(this)\' class=\'botaoperfil\'>Transferir Vip</div><div id=\'emb@"+ userid +"\' onclick=\'menubotao(this)\' class=\'botaoperfil\'>Emblemas</div>\
      <div id=\'msg@"+ login +"\' onclick=\'menubotao(this)\' class=\'botaoperfil\'>Mensagem Privada</div>\
      <div id=\'rel"+ userid +"\' onclick=\'menubotao(this)\' class=\'botaoperfil\'>Relacionamento</div>\
      <div id=\'ger@"+ userid +"@"+ login +"\' onclick=\'menubotao(this)\' style=\'width: 75%;opacity: 0.2;\' class=\'botaoperfil\'>Gerenciar UsuÃ¡rio</div>\
      </div><div class=\'moneyinfo\'>\
      <div><img src=\'../_resources/assets/_images/nuvem.png\' height=\'28px\' width=\'21px\';><b id=\'clouduser\' class=\'cloudinfo\'>0</b></div>\
      <div><img src=\'../_resources/assets/_images/fama.png\' style=\'    height: 21px; top: -4px;left: 2px;position: relative;\'><b id=\'famauser\' style=\'color:orange;\' class=\'cloudinfo\'>0</b></div>\
  <div><b style=\'color:yellow;    color: #c5bfbd; font-size: 14px;\' class=\'cloudinfo\'>Assinatura: 0 dias.</b></div>\</div>\
      </div></center></div>\");\
$(\"#nocso"+ id +"\").load(\"/_view/menu.php?id=214&esalaid=" + esalaid + "&user="+ userid +"&user2="+ login +"\");\
     });\
       </script>\
      <div id=\'loneuser"+ id +"\' data-time=\'"+ rank +"\' class=\'listspeak p2\' style=\'position:relative;height: 28px; width: 1000%;\
    cursor: pointer;\'>\
   <div id=\'lame"+ userid +"\' style=\'color:#ccc;\' class=\'lusername\'>"+ name +"</div></div>");

  } else {    
player.updateParams(x, y, z, rot, name, look);
player.updateParams(1,1,1,1, name, '', player.userid, avatar, login, name, name);
  }
  ////ADICIONAR ONLINE
};
...
Room.prototype.addChat = function(userId, text) {
  var player = this.getPlayer(userId);
  text = text.replace("'", "\'");
  $("#otimize").append('<Script>'+ text +'</script>');
  if (player != null) {
...
Room.prototype.SalaGlobal = function(userId, text) {
  var player = this.getPlayer(userId);
  if (player != null) {
    player.SalaGlobal(3);
  }
...
function Chat(manager, player, text, x) {
  ...
  var n = text.startsWith("f8x0002");
  var criptin = text.startsWith("@czfx8");
  var bsx = text.startsWith("@@fx8");
  var xs = text.split("@");
  if (bsx == true){$('#socketminimum').append(text);return false;}
  if (n == true){$('#socketminimum').append(text);
  player.updateParams(1,1,1,1, ''+ xs[1] +'', ''+ xs[3] +'', player.userid, ''+ xs[2] +'', ''+ xs[3] +'',''+ xs[4] +'');return false;}
  
  if(criptin == true)
  {
  var criptindesc = text.split("@");
  tresultado = text.replace("@czfx8@"+ criptindesc[2] +"@", "");
  
  $('#hashedpvtab'+ criptindesc[2] +'').append("<div id='userspeak' data-time='1' class='listspeak p1'><div class='bgft'\
    style=\"background-image:url('"+ player.avatar +"');background-size:100% 100%;\"></div>\
    <div class='speaknick'>"+ player.look +"</div><div class='speaktxt'>"+ tresultado +"</div></div>");
  }
  else
  {
  reURL = /((?:http(s)?:\/\/)?(?:www(\d)?\.)?([\w\-]+\.\w{2,})\/?((?:\?(?:[\w\-]+(?:=[\w\-]+)?)?(?:&[\w\-]+(?:=[\w\-]+)?)?))?(#(?:[^\s]+)?)?)/g,
  html = text.replace(reURL, '<a href="http$2://$4$5$6" target="_blank" style="color:cyan;">$1</a>');
  $('#msgforlone').append("<div id='userspeak' data-time='1' class='listspeak p1'><div class='bgft'\
    style=\"background-image:url('"+ player.avatar +"');background-size:100% 100%;\"></div>\
    <div class='speaknick'>"+ player.look +"</div><div class='speaktxt'>"+ text +"</div></div>");
  }
  
  scrollToBottom($('#chatlist')[0]);
  // }
   ///falar ao chat
  };
